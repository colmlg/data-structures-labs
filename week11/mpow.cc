...
void matrixPow(const Matrix& inMatrix, Matrix& result, int power);
...
int main(int argc, char *argv[]) {
    ...
    matrixPow(sparseMatrix, resultMatrix, power);
...
void matrixPow(const Matrix& inMatrix, Matrix& result, int power) {
	...
	while(power > 1) {
		...
		if(power % 2 == 0 && (power / 2) > 1) {
			multiplyMatrices(result, result, newResult);
			power /= 2;
		} else {
			multiplyMatrices(inMatrix, result, newResult);
			power--;
        }
        result = newResult;
	}
